...
#Filter GTDB_checkm_GLAMR_filter for WLE samples
WLE_GTDB_checkm_GLAMR_ada_passqc <- GTDB_checkm_GLAMR_ada_passqc %>%
                                        rename("sample.x" = "SampleID") %>%
                                        filter(SampleID %in% GLAMR_sample_table$SampleID)

#Gather all read paths to ada bins to cluster by nucleotide similarity (remove similar bins), sym link to new path
read_paths_ada_redo <- system("find /geomicro/data2/kiledal/GLAMR/data/omics/metagenomes/*/bins/all_raw_bins/ -path *.fa", intern=TRUE) %>% 
          tibble(read_path = .) %>% 
          mutate(bin_id = read_path %>% str_remove(".*all_raw_bins/") %>% str_remove(".fa.*"),
                 sample = read_path %>% str_remove(".*metagenomes/") %>% str_remove("/bins.*"),
                 new_path = str_glue("ada_bins_ada_redo/raw_bins_ada_redo/{bin_id}.fa")) %>%
          filter(sample %in% WLE_GTDB_checkm_GLAMR_ada_passqc$SampleID &
                 bin_id %in% WLE_GTDB_checkm_GLAMR_ada_passqc$`Bin Id`)

file.symlink(read_paths_ada_redo$read_path, read_paths_ada_redo$new_path) 

```

9.) drep ADA bins redo
```{bash}

###conda config --set channel_priority strict
snakemake run_ada_drep_ada_redo --profile config/snakemake_profiles/alpena/ --keep-going 
