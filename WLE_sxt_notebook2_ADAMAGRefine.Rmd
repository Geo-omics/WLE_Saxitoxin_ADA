...
snakemake run_gtdbtk_GToTree --profile config/snakemake_profiles/alpena --rerun-incomplete 
...
2.) GToTree preparation initial (drep 95, 97, 98, 99)
```{r}
...
4.) Setup terminal and screen session to run and visualize Anvi'o
```{bash}
...
5.) Run snakemake file to setup and generate Anvi'o sample profiles
```{bash}
...
17.) Merge Anvi'o profiles, import collections, interact, visualize, and refine bins
samp_4380
```{bash}
...
18.) Merge Anvi'o profiles, import collections, interact, visualize, and refine bins
samp_507
```{bash}
...
19.) Merge Anvi'o profiles, import collections, interact, visualize, and refine bins
samp_4305
```{bash}
...
20.) Merge Anvi'o profiles, import collections, interact, visualize, and refine bins
samp_2059
```{bash}
...
21.) Examine refined bins with checkm
```{bash}
...
22.) Compare new checkM results to old
```{r}
...
23.) drep_99 and run GTDBTK on MAGs one last time for confirmation (performed on the final final MAGS!)
```{bash}
...
24.) Prep for (re-re)run of GTDBTK on MAGs/Genomes for tree building - future labels
```{r}
...
25.) GToTree preparation ADA_redo (drep 98, 99)
```{r}
...
26.) Make GToTree(s) with FINAL MAGs
```{bash}
